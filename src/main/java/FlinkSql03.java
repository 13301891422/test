...
//            ")";
    ...
    public static final String MYSQL_TABLE_DIM_DDL = ""+
            ...
            "  'connector.url' = 'jdbc:mysql://hadoop105:3306/test', -- jdbc的url\n" +
    ...
    public static final String MYSQL_TABLE_SINK_DDL=""+
            ...
            "  'connector.write.flush.interval' = '1' -- 意思是攒满多少秒才触发写入；这2个参数，" +
            "无论数据满足哪个条件，就会触发写入\n" +
//            "  'update-mode' = 'upsert' -- 指定为插入更新模式 \n"+
    ...
    public static void main(String[] args) throws Exception {
        ...
        //计算每种类型的行为有多少用户
        Table group = tEnv.sqlQuery
                ("select behavior,count(distinct user_id) count_unique_user " +
                        "from source_kafka group by behavior ");

        //转回datastream，因为需要增加proctime，而目前定义proctime方式只有两种，
        // 一种是在定义DDL的时候，一种是在DataStream转 Table的时候
        ...
        Table join = tEnv.sqlQuery("select " +
                "b.c_name as behavior , " +
                "max(a.count_unique_user) ," +
                "a.behavior as e_name " +
